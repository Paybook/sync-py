...
class Paybook():
    ...
    def __call__(endpoint=None, method=None, data=None, params=None, headers=None, url=None):
        ...
        Paybook.log(Paybook.__INDENT__ + 'API Key:     ' +
                    str(Paybook.api_key), call=True)
        Paybook.log(Paybook.__INDENT__ + 'Endpoint:    ' + str(url), call=True)
        Paybook.log(Paybook.__INDENT__ + 'HTTP Method: ' +
                    str(method), call=True)
        Paybook.log(Paybook.__INDENT__ +
                    'Data:        ' + str(data), call=True)
        Paybook.log(Paybook.__INDENT__ + 'Params:      ' +
                    str(params), call=True)
        Paybook.log(Paybook.__INDENT__ + 'Headers:     ' +
                    str(headers), call=True)
        if method == 'post':
            conn = requests.post(
            ...
            conn = requests.delete(
                url, data=data, params=params, headers=headers)
        Paybook.log(Paybook.__INDENT__ + 'API Response code: ' +
                    str(conn.status_code), call=True)
